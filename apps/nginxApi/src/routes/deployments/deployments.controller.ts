...
import { NginxDeploymentResponseType, NginxDeploymentType, ResponseType } from 'shared/src/types';
...
export class DeploymentsController {
	...
	@Post()
	async create(@Body() deploymentConfiguration: NginxDeploymentType): Promise<ResponseType> {
	...
	async get(@Query('id') id: string): Promise<NginxDeploymentResponseType> {
		if (!id) throw new BadRequestException({ success: false, message: 'No id provided' });
